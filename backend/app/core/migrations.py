...
def run_lightweight_migrations() -> None:
    if engine.url.drivername.startswith("sqlite"):
        statements = [
            ...
            "ALTER TABLE customers ADD COLUMN is_task_favorite BOOLEAN NOT NULL DEFAULT 0",
            "CREATE INDEX ix_customers_is_task_favorite ON customers (is_task_favorite)",
            ...
            "ALTER TABLE sessions ADD COLUMN contact_scan_status VARCHAR(30) NOT NULL DEFAULT 'idle'",
            "ALTER TABLE sessions ADD COLUMN contact_scan_detail TEXT NULL",
            "CREATE INDEX ix_sessions_contact_scan_status ON sessions (contact_scan_status)",
            ...
            "ALTER TABLE materials ADD COLUMN health_status VARCHAR(30) NOT NULL DEFAULT 'active'",
            "ALTER TABLE materials ADD COLUMN success_count INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE materials ADD COLUMN failure_count INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE materials ADD COLUMN consecutive_failures INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE materials ADD COLUMN last_error TEXT NULL",
            "ALTER TABLE materials ADD COLUMN last_failed_at DATETIME NULL",
            "ALTER TABLE materials ADD COLUMN disabled_at DATETIME NULL",
            "CREATE INDEX ix_materials_health_status ON materials (health_status)",
            "ALTER TABLE messages ADD COLUMN source VARCHAR(30) NOT NULL DEFAULT 'telegram_sync'",
            "ALTER TABLE messages ADD COLUMN task_id INTEGER NULL",
            "CREATE INDEX ix_messages_source ON messages (source)",
            "CREATE INDEX ix_messages_task_id ON messages (task_id)",
            "CREATE INDEX ix_messages_session_chat_source_created ON messages (session_id, chat_id, source, created_at)",
        ]
    ...
    statements = [
        ...
        "CREATE INDEX ix_customers_is_favorite ON customers (is_favorite)",
        "ALTER TABLE customers ADD COLUMN is_task_favorite BOOLEAN NOT NULL DEFAULT FALSE",
        "CREATE INDEX ix_customers_is_task_favorite ON customers (is_task_favorite)",
        "ALTER TABLE sessions ADD COLUMN bidirectional_status VARCHAR(30) NOT NULL DEFAULT 'unchecked'",
        "ALTER TABLE sessions ADD COLUMN bidirectional_detail TEXT NULL",
        "ALTER TABLE sessions ADD COLUMN last_bidirectional_check_at DATETIME NULL",
        ...
        "UPDATE users SET menu_permissions = '[\"messages\",\"materials\"]' WHERE menu_permissions IS NULL",
        "ALTER TABLE materials ADD COLUMN health_status VARCHAR(30) NOT NULL DEFAULT 'active'",
        "ALTER TABLE materials ADD COLUMN success_count INT NOT NULL DEFAULT 0",
        "ALTER TABLE materials ADD COLUMN failure_count INT NOT NULL DEFAULT 0",
        "ALTER TABLE materials ADD COLUMN consecutive_failures INT NOT NULL DEFAULT 0",
        "ALTER TABLE materials ADD COLUMN last_error TEXT NULL",
        "ALTER TABLE materials ADD COLUMN last_failed_at DATETIME NULL",
        "ALTER TABLE materials ADD COLUMN disabled_at DATETIME NULL",
        "CREATE INDEX ix_materials_health_status ON materials (health_status)",
        "ALTER TABLE messages ADD COLUMN source VARCHAR(30) NOT NULL DEFAULT 'telegram_sync'",
        "ALTER TABLE messages ADD COLUMN task_id INT NULL",
        "CREATE INDEX ix_messages_source ON messages (source)",
        "CREATE INDEX ix_messages_task_id ON messages (task_id)",
        "CREATE INDEX ix_messages_session_chat_source_created ON messages (session_id, chat_id, source, created_at)",
        "ALTER TABLE messages ADD CONSTRAINT fk_messages_task FOREIGN KEY (task_id) REFERENCES tasks(id) ON DELETE SET NULL",
    ]
    with engine.begin() as connection:
        for statement in statements:
            try:
